...
export class ArchivedConvocationDetailView extends React.Component<Props> {
	render() {
		...
		if (event.enclosure) {
			let url = event.enclosure.url
			let length = event.enclosure.length

			eventLink = getConvoLink(url)

			let duration = moment.duration(parseInt(length) / 100)
